...
import { ArrowRight02Icon, Calendar02Icon, Clock01Icon } from "@hugeicons/core-free-icons";
				...
				<Link href={`/blog/${post.slug}`} className="group block">
					<div className="grid lg:grid-cols-2 gap-0 rounded-2xl border border-border bg-muted/30 overflow-hidden">
						<div className="aspect-video lg:aspect-auto bg-primary/5 flex items-center justify-center min-h-[240px]">
							<HugeiconsIcon icon={post.icon} className="size-16 text-primary/30" strokeWidth={1.5} />
						</div>
						<div className="p-8 lg:p-12 flex flex-col justify-center">
							...
							<div className="mt-6 flex items-center gap-4 text-sm text-muted-foreground">
								<span className="flex items-center gap-1.5">
									<HugeiconsIcon icon={Calendar02Icon} className="size-4" strokeWidth={1.5} />
							...
							<div className="mt-6">
								<span className="inline-flex items-center gap-2 text-primary font-medium group-hover:gap-3 transition-all">
									Read article
									<HugeiconsIcon icon={ArrowRight02Icon} className="size-4" strokeWidth={1.5} />
								</span>
							</div>
						</div>
					</div>
